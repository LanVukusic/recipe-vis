export function tokenizeSearch(s) {
  return s.toLowerCase().trim().split(/\s+/).filter(Boolean);
}

export function uniqueCuisines(index) {
  return Array.from(new Set(index.map(d => d.cuisine))).sort();
}

export function filterList(allIndex, cuisineValue, searchValue) {
...
export function fillRecipeSelect(selectEl, list, keepId = null) {
  selectEl.innerHTML = "";
  ...
  if (keepId !== null) {
    ...
    if (exists) selectEl.value = keepId;
  }
}

export function fillCuisineSelect(selectEl, cuisines) {
  selectEl.innerHTML = "";
...
export async function fetchJSON(url, niceNameForError) {
  const resp = await fetch(url);
  if (!resp.ok) throw new Error(`Could not load ${niceNameForError} (${url}): ${resp.status}`);
