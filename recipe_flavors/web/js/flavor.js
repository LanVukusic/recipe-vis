...
export function pairWeight(flavorMap, a, b) {
  const k = canonKey(a, b);
  if (!k) return 0;
  return flavorMap?.[k] ?? 0;
}

export function computeScoreAvg(flavorMap, ings) {
  const n = ings.length;
  if (n < 2) return 0;
  let sum = 0;
  let cnt = 0;
...
export function computeCoverage(flavorMap, ings) {
  const n = ings.length;
  if (n < 2) return 0;
  let found = 0;
  let cnt = 0;
...
export function buildHeatmapLong(flavorMap, ings) {
  const out = [];
  for (const x of ings) {
    for (const y of ings) {
...
export function buildContributions(flavorMap, ings) {
  const out = [];
  for (const a of ings) {
    let sum = 0;
    let cnt = 0;
